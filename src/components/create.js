    ...
    <div className="container">
      <div className="row justify-content-center">
        <div className="col-10 col-lg-8">
          <h2 className="mb-4 text-center mt-3">Crear recurso</h2>
          <form onSubmit={(event) => props.submitCreate(props.titulo, props.claves, props.descripcion, props.fuente, props.tipoDelRecurso, props.cobertura, event)} className="mb-5">
            ...
            <button type="submit" className="btn btn-info">Crear recurso</button>
            <Link to="/" className="btn btn-info ml-2">Cancelar</Link>
          </form>
        </div>
      </div>
    </div>
...
const mapDispatchToProps = dispatch => {
  return {
    submitCreate(titulo, claves, descripcion, fuente, tipoDelRecurso, cobertura, event){
      event.preventDefault();
      dispatch(submitCreate(titulo, claves, descripcion, fuente, tipoDelRecurso, cobertura))
    },
    change(event){
